...
$to = $_GET['to'];
$from = $_GET['from'];
$stmt = $connect->prepare("SELECT * FROM chat WHERE (msg_from = ? AND msg_to = ? AND pro_id=?) OR (msg_to = ? AND msg_from=? AND pro_id=?) ORDER BY id ");
$stmt->execute(array($from, $to, $pro_id,  $from, $to, $pro_id));
$count = $stmt->rowCount();
if ($count > 0) {
